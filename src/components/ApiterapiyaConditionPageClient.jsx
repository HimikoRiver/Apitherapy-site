...
function SectionHeader({ icon, eyebrow, title }) {
  const Icon = ICON_MAP[icon] ?? History;
...
function StepsSection({ section }) {
  const StepIcon = ICON_MAP[section.itemIcon] ?? CheckCircle2;

  return (
    <div className="h-full rounded-[30px] border border-amber-200/10 bg-[#100d0b]/84 p-6 shadow-[0_18px_60px_rgba(0,0,0,0.24)] backdrop-blur-sm sm:p-8">
      <SectionHeader
        icon={section.icon}
        eyebrow={section.eyebrow}
        title={section.title}
...
function FaqSection({ section }) {
  return (
    <section className="rounded-[32px] border border-white/10 bg-[#0b0b10]/80 p-6 shadow-[0_20px_80px_rgba(0,0,0,0.28)] backdrop-blur-sm sm:p-8 md:p-10">
      <SectionHeader
        icon={section.icon}
        eyebrow={section.eyebrow}
        title={section.title}
...
export default function ApiterapiyaConditionPageClient({ pageData }) {
  ...
  for (let i = 0; i < sections.length; i += 1) {
    ...
    if (current.layout === "half") {
      ...
      if (next?.layout === "half") {
        pairedRows.push([current, next]);
        i += 1;
      } else {
        pairedRows.push([current]);
      }

      continue;
    }

    fullSections.push(current);
  }
  ...
  return (
    <main className="relative min-h-screen bg-[#050507] text-white">
      ...
      <section className="relative z-10 mx-auto w-full max-w-7xl px-6 pb-3 pt-8 md:px-10 lg:px-16">
        <div className="mb-6">
          <Link
            href="/#services"
            ...
            <div className="relative z-10 flex h-full flex-col">
              ...
              <div className="mt-5 space-y-4 text-sm leading-7 text-white/78">
                {aside.paragraphs.map((paragraph, index) => (
                  <p key={`aside-paragraph-${index}`}>{paragraph}</p>
                ))}
              </div>

              <div className="mt-6 rounded-[22px] border border-amber-200/12 bg-[#17110d]/75 p-4">
                <p className="text-sm leading-7 text-amber-50/90">{aside.note}</p>
              </div>
              ...
              <div className="mt-auto flex flex-col gap-3 pt-4">
                ...
                >
                  {aside.primaryButton.label}
                </Link>

                <Link
                  href={aside.secondaryButton.href}
